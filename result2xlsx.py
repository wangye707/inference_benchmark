...
def parse_args():
    ...
    parser.add_argument("--docker_name", type=str,
                        ...
                        help="docker name")
    parser.add_argument("--paddle_version", type=str, default="0.0.0", help="paddle_tag/paddle_commit")
    parser.add_argument("--onnxruntime_version", type=str, default="0.0.0", help="onnxruntime_tag/onnxruntime_commit")
...
def data_merging(env, paddle_version, onnxruntime_version, model2onnx_name_list,output_total_list):
    ...
    for output_dict_num in range(len(output_total_list)):
        ...
        for comp_list in output_total_list[output_dict_num:]:
            if output_dict["model_name"] == comp_list["model_name"] and \
                ...
                log_dict["onnxruntime_cpu_mem"] = "0"
                log_dict["paddle_cpu_mem"] = "0"
                ...
                log_dict["cpu_mem_gap"] = calculation_gap(paddle_num=log_dict["paddle_cpu_mem"],
                                                          onnxruntime_num=log_dict["paddle_cpu_mem"])
                log_dict["gpu_mem_gap"] = calculation_gap(paddle_num=log_dict["paddle_gpu_mem"],
                                                          onnxruntime_num=log_dict["onnxruntime_gpu_mem"])
                log_dict["perf_mem_gap"] = calculation_gap(paddle_num=log_dict["paddle_avg_cost"],
                                                           onnxruntime_num=log_dict["onnxruntime_avg_cost"])
                log_dict["paddle2onnx_model_convert"] = "False"
                for model2onnx_success_log in model2onnx_name_list:
                    if log_dict["model_name"] in model2onnx_success_log and "success" in model2onnx_success_log:
                        log_dict["paddle2onnx_model_convert"] = "True"
                        break
            else:
                continue
...
def calculation_gap(paddle_num, onnxruntime_num):
    if float(paddle_num) <= 0 and float(onnxruntime_num) <= 0:
        ...
        return str((float(paddle_num) - float(onnxruntime_num)) / float(onnxruntime_num))
...
def main(args, result_path, tipc_benchmark_excel_path):
    ...
    origin_df = pd.DataFrame(columns=["日期", "软件环境", "paddle_version", "onnxruntime_version",
                                      "device_name", "model_name", "precision", "batch_size", "enable_mkldnn",
                                      "cpu_threads", "enable_gpu", "enable_trt",
                                      "paddle2onnx_model_convert", "onnxruntime_cpu_mem", "paddle_cpu_mem",
                                      "paddle_gpu_mem", "onnxruntime_gpu_mem", "onnxruntime_avg_cost",
                                      "paddle_avg_cost", "cpu_mem_gap", "gpu_mem_gap", "perf_mem_gap"])
    ...
    output_excl_list = data_merging(env, paddle_version, onnxruntime_version, model2onnx_name_list,dict_list)
